...
type VerifyState =
  | { status: "idle" }
  | { status: "running" }
  | { status: "done"; clientMatch: boolean; serverMatch: boolean | null; recomputed: string };
...
export function ReceiptCard({ receipt }: { receipt: Receipt }) {
  ...
  async function runVerify() {
    ...
    const client = await verifyReceiptHash(receipt as unknown as Record<string, unknown>);
    let serverMatch: boolean | null = null;
    try {
      const s = await api<{ match: boolean }>(`/receipts/${receipt.receiptId}/verify`);
      serverMatch = s.match;
    } catch {
      serverMatch = null;
    }
      ...
      {verify.status === "done" && (
        <div className="mt-3 text-xs mono space-y-1 border-t border-[var(--color-edge)] pt-3">
          <div className="flex gap-2">
            <span className="text-[var(--color-ink-3)] w-24 shrink-0">client sha256</span>
            <span className={verify.clientMatch ? "text-[var(--color-pass)]" : "text-[var(--color-fail)]"}>
              {verify.clientMatch ? "MATCH" : "MISMATCH"} — recomputed {verify.recomputed.slice(0, 18)}…
            </span>
          </div>
          <div className="flex gap-2">
            <span className="text-[var(--color-ink-3)] w-24 shrink-0">server verify</span>
            <span className={
              verify.serverMatch === null ? "text-[var(--color-ink-3)]" :
              verify.serverMatch ? "text-[var(--color-pass)]" : "text-[var(--color-fail)]"
            }>
              {verify.serverMatch === null ? "unavailable" : verify.serverMatch ? "MATCH" : "MISMATCH"}
            </span>
          </div>
        </div>
      )}
