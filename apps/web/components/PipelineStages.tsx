...
const LABEL: Record<string, string> = {
  ...
  INTENT_COMPILED: "Compiled",
  DISCOVERING: "Representations",
  QUOTING: "Quotes",
  POLICY_EVALUATION: "Policy",
  SIMULATING: "Simulation",
  AWAITING_CONFIRMATION: "Authorization",
  READY: "Ready",
  EXECUTING: "Execution",
  PENDING_CONFIRMATION: "On-chain",
  CONFIRMED: "Confirmed",
  NO_VALID_ROUTE: "No valid route",
  ...
  return (
    <div className="panel p-4">
            ...
            <div key={s} className="flex items-center gap-1 shrink-0">
              <div
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs ${
                  seen
                    ? "bg-[var(--color-panel-2)] text-ink"
                    : "text-[var(--color-ink-3)]"
                } ${isFinal && !failed ? "border border-[var(--color-accent)]" : ""}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${seen ? "bg-[var(--color-pass)]" : "bg-[var(--color-edge)]"}`}
                />
                {LABEL[s] ?? s}
              </div>
              {i < ORDER.length - 1 && (
      ...
      <div className="mt-3 text-xs text-[var(--color-ink-2)] flex items-center gap-2">
        final state:
        <span
          className={`chip ${failed ? "chip-fail" : state === "CONFIRMED" ? "chip-pass" : "chip-warn"}`}
        >
          {state}
        </span>
