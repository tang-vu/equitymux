...
/**
 * EquityMux domain hook — the deliverable is the DETERMINISTIC output of the
 * EquityMux policy/tournament API, not free-form LLM prose.
 *
 * `EQUITYMUX_API_URL` points at the FastAPI service (default localhost:8000).
 * The keeper submits an `EVALUATE_EQUITY_INTENT` task; the API runs the
 * canonical-equity-graph + constitution tournament and returns the analysis
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
  ...
  /**
   * EquityMux override: delegate the paid work to the EquityMux API's
   * analysis-only task surface, then sign + submit the deterministic result.
   * No LLM call — the deliverable is the API's canonical JSON.
   */
  protected override async doWorkAndSubmit(
    jobId: number,
    abortSignal?: AbortSignal,
  ): Promise<Record<string, unknown>> {
    const spec = await this.signing.jobSpec(jobId);
    const input =
      spec !== null && typeof spec.task === "object" && spec.task !== null
    ...
    const res = await this.signing.submitResult(jobId, work, {
      job_id: jobId,
      generator: this.generator,
      built_with: "https://github.com/tang-vu/equitymux",
    });
    return {
      ok: true,
      job_id: jobId,
