...
class BinancePublicClient:
    ...
    def stock_list(self, platform_type: int | None = None) -> list[dict]:
        ...
        return self._cached(
            f"stock-list:{platform_type}",
            60,
            lambda: (self._get(RWA_LIST, params, module="rwa").get("data")) or [],
        )
    ...
    def rwa_meta(self, chain_id: int, contract: str) -> dict:
        ...
        return self._cached(
            f"meta:{chain_id}:{contract}",
            120,
            lambda: (
                self._get(
                    RWA_META, {"chainId": str(chain_id), "contractAddress": contract}, module="rwa"
                ).get("data")
                or {}
            ),
        )
    ...
    def market_status(self) -> dict:
        ...
        return self._cached(
            "market-status", 15, lambda: self._get(RWA_MARKET_STATUS, module="rwa").get("data") or {}
        )
    ...
    def asset_market_status(self, chain_id: int, contract: str) -> dict:
        ...
        return self._cached(
            f"asset-status:{chain_id}:{contract}",
            15,
            lambda: (
                self._get(
                    RWA_ASSET_STATUS, {"chainId": str(chain_id), "contractAddress": contract}, module="rwa"
                ).get("data")
                or {}
            ),
        )
    ...
    def rwa_dynamic(self, chain_id: int, contract: str) -> dict:
        ...
        return self._cached(
            f"dyn:{chain_id}:{contract}",
            10,
            lambda: (
                self._get(
                    RWA_DYNAMIC_V2, {"chainId": str(chain_id), "contractAddress": contract}, module="rwa"
                ).get("data")
                or {}
            ),
        )
    ...
    def token_kline(self, chain_id: int, contract: str, interval: str = "1d", limit: int = 30) -> dict:
        ...
        return (
            self._get(
                TOKEN_KLINE,
                {"chainId": str(chain_id), "contractAddress": contract, "interval": interval, "limit": limit},
                module="market",
            ).get("data")
            or {}
        )
    ...
    def token_dynamic(self, chain_id: int, contract: str) -> dict:
        if self.s.demo_mode:
            try:
                ...
                return {}
        return self._cached(
            f"tokdyn:{chain_id}:{contract}",
            15,
            lambda: (
                self._get(
                    TOKEN_DYNAMIC, {"chainId": str(chain_id), "contractAddress": contract}, module="market"
                ).get("data")
                or {}
            ),
        )
    ...
    def token_search(self, chain_id: int, keyword: str) -> dict:
        if self.s.demo_mode:
            try:
                ...
                return {}
        return (
            self._get(TOKEN_SEARCH, {"chainId": str(chain_id), "keyword": keyword}, module="market").get(
                "data"
            )
            or {}
        )
    ...
    def token_audit(self, chain_id: int, contract: str) -> dict:
        ...
        body = {"binanceChainId": str(chain_id), "contractAddress": contract, "requestId": str(uuid.uuid4())}
