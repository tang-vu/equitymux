...
class BscRpc:
    ...
    def _call(self, method: str, params: list[Any]) -> Any:
        ...
        for url in self.s.rpc_urls:
            ...
            try:
                ...
                record_event(module="bsc-rpc", endpoint=url.split("//")[-1].split("/")[0],
                             operation=method, method="POST",
                             http_status=r.status_code, latency_ms=latency, success=ok,
    ...
    def simulate_call(self, to: str, data: str, value_wei: int = 0,
        ...
        if from_addr:
            tx["from"] = from_addr
        try:
            result = self.eth_call(tx["to"], tx["data"])
