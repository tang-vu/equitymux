...
def run_task(kind: str, inp: dict[str, Any], pipeline) -> dict:
    ...
    if kind == "PREPARE_REBALANCE":
        return {"status": "PREPARED_NOT_EXECUTED",
...
def evaluate_equity_intent(inp: dict[str, Any], pipeline) -> dict:
    ...
    intent = EquityIntent(raw=f"agent eval {ticker}", ticker=ticker, side=Side.BUY,
...
def check_compliance(inp: dict[str, Any], pipeline) -> dict:
    ...
    if pct is not None and total > 0:
        for ticker, usd in exposures.items():
            ...
            if share > pct:
                violations.append({"rule": "concentration.max_single_underlying",
                                   "ticker": ticker, "sharePct": str(share),
                                   "maxPct": str(pct)})
    return {"compliant": not violations, "violations": violations,
            "constitutionHash": row["hash"]}
