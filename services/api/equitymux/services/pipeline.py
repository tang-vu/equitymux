...
class Pipeline:
    def __init__(self, settings: Settings | None = None):
        ...
        self.graph = CanonicalEquityGraph(client=BinancePublicClient(self.s), platforms=self.s.platforms)
    ...
    def run(
        self,
        intent: EquityIntent,
        constitution: PortfolioConstitution,
        ...
        *,
        ...
        if not reps:
            ...
            rec = self._receipt(
                sm, intent, chash, [], self._market_ctx(), [], None, None, {}, {}, execution_mode
            )
        ...
        sm.transition(ExecState.POLICY_EVALUATION, note=f"{len(candidates)} candidates evaluated")
        eligible = [
            c for c in candidates if c.status in (RouteStatus.ELIGIBLE, RouteStatus.REQUIRES_CONFIRMATION)
        ]
        if not eligible:
            sm.transition(
                ExecState.NO_VALID_ROUTE, note="all candidates rejected by policy or missing quotes"
            )
            rec = self._receipt(
                sm,
                intent,
                chash,
                self._checks(candidates),
                self._market_ctx(),
                candidates,
                None,
                None,
        ...
        needs_confirm = (
            selected.policy.requires_confirmation if selected.policy else True
        ) or self.s.require_confirmation
        if needs_confirm and not confirm:
            sm.transition(
                ExecState.AWAITING_CONFIRMATION, note="policy/wallet requires explicit confirmation"
            )
            rec = self._receipt(
                sm,
                intent,
                ...
                selected,
                sim.model_dump(mode="json"),
                {"equityMuxPolicy": "PASS", "humanConfirmationRequired": True},
                {},
                execution_mode,
            )
            persistence.save_receipt(rec)
        ...
        if not self.s.execution_enabled:
            sm.transition(ExecState.POLICY_REJECTED, note="EXECUTION_ENABLED=false (system kill switch)")
            rec = self._receipt(
                sm,
                intent,
                chash,
                ...
                selected,
                sim.model_dump(mode="json"),
                {"equityMuxPolicy": "PASS", "systemKillSwitch": "EXECUTION_ENABLED=false"},
                {},
                execution_mode,
            )
            persistence.save_receipt(rec)
        ...
        if execution.get("status") == "CONFIRMED":
            ...
            sm.transition(ExecState.CONFIRMED, note=f"order {execution.get('orderId')} FINISHED")
            ...
            sm.transition(
                ExecState.PENDING_CONFIRMATION, note=f"order {execution.get('orderId')} pending past 90s poll"
            )
        else:
            sm.transition(ExecState.EXECUTION_FAILED, note=execution.get("error", "execution failed"))
        rec = self._receipt(
            sm,
            intent,
            chash,
    ...
    # ---------------- internals ----------------
    def _simulate(self, intent: EquityIntent, c: CandidateRoute) -> SimulationRecord:
        """The current wallet quote does not bind the later swap calldata.

        A balance probe cannot establish swap success. Fail closed until the
        adapter can simulate and submit the identical transaction envelope.
        """
        return SimulationRecord(
            status="SKIP",
            method="unavailable",
            timestamp=_iso(),
            detail="BLOCKED: wallet quote does not expose a bound swap transaction; "
            "balance checks are not transaction simulation",
        )

    def _execute(self, intent: EquityIntent, c: CandidateRoute, sim: SimulationRecord) -> dict:
        if self.s.demo_mode or not self.s.execution_enabled:
            return {"status": "FAILED", "error": "execution disabled or recorded data"}
        if sim.status != "PASS" or not sim.calldata_hash:
            return {"status": "FAILED", "error": "no bound transaction simulation"}
        ...
        if from_token is None:
            return {"status": "FAILED", "error": f"quote asset {intent.quote_asset} not in allowlist"}
        slip = str((c.expected_slippage_bps or 50) / 100)
        try:
            res = self.wallet.swap(
                from_token, c.representation.token_address, str(intent.notional), "56", slippage=slip
            )
        ...
        while time.time() < deadline:
            ...
            if order and order.get("status") in ("FINISHED", "FAILED"):
                out = {
                    "status": "CONFIRMED" if order["status"] == "FINISHED" else "FAILED",
                    "orderId": order_id,
