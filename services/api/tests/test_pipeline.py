...
def _attach_quote(intent, candidates):
    ...
    for c in candidates:
        c.quote = ExecutableQuote(
            ...
            from_symbol="USDC", to_symbol=c.representation.token_symbol,
            from_amount=intent.notional or Decimal(10),
            to_amount=Decimal("0.05"), slippage_bps=30,
            obtained_at=_iso(), source="test", raw={},
        )


def _stub_sim(intent, c):
    return SimulationRecord(status="PASS", method="test",
                            timestamp=_iso(), detail="stub simulation")
...
def test_eligible_path_reaches_kill_switch(settings, rep_ondo, intent, state, constitution):
    ...
    assert states.index("QUOTING") < states.index("POLICY_EVALUATION") \
        < states.index("SIMULATING")


def test_no_notional_marks_no_quote_not_eligible(settings, rep_ondo, state, constitution):
    p = Pipeline(settings)
    p.graph.discover = lambda ticker, enrich=True: [rep_ondo]
    intent = EquityIntent(raw="buy NVDA exposure", ticker="NVDA",
                          side=Side.BUY, notional=None, quote_asset="USDC")
    ...
    sell = EquityIntent(raw="sell NVDA", ticker="NVDA", side=Side.SELL,
