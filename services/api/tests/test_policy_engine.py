...
from equitymux.domain.models import (
    EquityIntent, MarketState, RouteStatus, SimulationRecord,
)
...
class TestReserve:
    def test_passes_when_reserve_kept(self, rep_ondo):
        ...
        r = [x for x in ev.results if x.rule == "reserve.min_quote_reserve"][0]
...
class TestPremium:
    def test_premium_within_cap(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "execution.max_premium"][0].status == "PASS"

    def test_premium_over_cap_fails(self, rep_ondo):
        e = eng(execution={"max_premium_bps": Decimal("40")})
        ev = e.evaluate(intent(), cand(rep_ondo, premium=62), BASE_STATE)
        r = [x for x in ev.results if x.rule == "execution.max_premium"][0]
        assert r.status == "FAIL"
...
class TestSlippage:
    def test_slippage_cap(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "execution.max_slippage"][0].status == "FAIL"
...
class TestConcentration:
    def test_concentration_fail(self, rep_ondo):
        ...
        assert [x for x in ev.results if "concentration" in x.rule][0].status == "FAIL"
...
class TestMarketHours:
    def test_closed_requires_confirm(self, rep_xstocks):
        ...
        r = [x for x in ev.results if x.rule == "market_hours.confirm_when_closed"][0]
    ...
    def test_closed_blocked_when_disallowed(self, rep_xstocks):
        ...
        ev = e.evaluate(intent(), cand(rep_xstocks), BASE_STATE)
        assert [x for x in ev.results if x.rule == "market_hours.allow_when_closed"][0].status == "FAIL"
    ...
    def test_halted_blocked(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "market_hours.block_when_halted"][0].status == "FAIL"
...
class TestStaleness:
    def test_stale_reference_fails(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "reference.max_age"][0].status == "FAIL"

    def test_missing_reference_fails(self, rep_ondo):
        rep_ondo.reference_price_usd = None
        e = eng()
        ev = e.evaluate(intent(), cand(rep_ondo), BASE_STATE)
        assert [x for x in ev.results if x.rule == "reference.require_reference_price"][0].status == "FAIL"
...
class TestRepresentation:
    def test_platform_not_allowed(self, rep_xstocks):
        ...
        ev = e.evaluate(intent(), cand(rep_xstocks), BASE_STATE)
        assert [x for x in ev.results if x.rule == "representation.allowed_platforms"][0].status == "FAIL"

    def test_contract_not_allowed(self, rep_ondo):
        e = eng(representation={"allowed_contracts": ["0xdead"]})
        ev = e.evaluate(intent(), cand(rep_ondo), BASE_STATE)
        assert [x for x in ev.results if x.rule == "representation.allowed_contracts"][0].status == "FAIL"
    ...
    def test_attestation_required(self, rep_ondo):
        ...
        ev = e.evaluate(intent(), cand(rep_ondo), BASE_STATE)
        assert [x for x in ev.results if x.rule == "representation.require_attestation"][0].status == "FAIL"
...
class TestSimulation:
    def test_failed_sim_fails(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "execution.require_simulation"][0].status == "FAIL"
...
class TestSystemCeilings:
    def test_system_notional_cap_beats_constitution(self, rep_ondo):
        ...
        assert [x for x in ev.results if x.rule == "system.max_notional"][0].status == "FAIL"
        assert not ev.eligible
    ...
    def test_wrong_chain_fails(self, rep_ondo):
        ...
        e = eng()
        ev = e.evaluate(intent(), cand(rep_ondo), BASE_STATE)
        assert [x for x in ev.results if x.rule == "system.chain_id"][0].status == "FAIL"
